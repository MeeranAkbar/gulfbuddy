...
create policy "Public view jobs" on jobs for select using (status = 'active');
create policy "Users insert jobs" on jobs for insert with check (auth.uid() = user_id);
create policy "Users update jobs" on jobs for update using (auth.uid() = user_id);
create policy "Users delete jobs" on jobs for delete using (auth.uid() = user_id);
