...
const verticals = [
  { 
    ...
    importFunc: 'getJobListingBySlug',
    importPath: '../../../../lib/search/jobs',
    specs: [
      { label: 'Type', valueKey: 'seniorityLevel' },
      { label: 'Experience', valueKey: 'seniorityLevel' },
      { label: 'Location', valueKey: 'location' }
    ]
  },
  { 
    name: 'classifieds', 
    ...
    titleKey: 'title',
    priceKey: 'priceLabel',
    importFunc: 'getClassifiedsListingBySlug',
    importPath: '../../../../lib/search/classifieds',
    specs: [
      { label: 'Condition', valueKey: 'condition' },
      { label: 'Location', valueKey: 'location' },
      { label: 'Delivery', valueKey: 'delivery' }
    ]
  },
  { 
    name: 'services', 
    ...
    titleKey: 'title',
    priceKey: 'priceLabel',
    importFunc: 'getServicesListingBySlug',
    importPath: '../../../../lib/search/services',
    specs: [
      { label: 'Availability', valueKey: 'availability' },
      { label: 'Location', valueKey: 'location' },
      { label: 'Rating', valueKey: 'rating' }
    ]
  },
  { 
    name: 'directory', 
    ...
    titleKey: 'title',
    priceKey: 'priceLabel',
    importFunc: 'getDirectoryListingBySlug',
    importPath: '../../../../lib/search/directory',
    specs: [
...
const appDir = path.join(__dirname, 'apps', 'web', 'app', '(public)');

verticals.forEach(v => {
  const dirPath = path.join(appDir, v.name, `[${v.slugKey}]`);
  ...
  const template = `import { notFound } from 'next/navigation';
import { ${v.importFunc} } from '${v.importPath}';
import Link from 'next/link';
import { ChevronLeftIcon, ShareIcon, HeartIcon } from '@heroicons/react/24/outline';
import { CheckBadgeIcon } from '@heroicons/react/24/solid';

interface Props {
  params: Promise<{ ${v.slugKey}: string }>;
}

export default async function DetailPage({ params }: Props) {
  const resolvedParams = await params;
  const listing = ${v.importFunc}(resolvedParams.${v.slugKey});
    ...
    <div className="bg-[var(--background)] pb-24 animate-in fade-in duration-700">
      ...
      <div className="sticky top-0 z-50 border-b border-[var(--border-subtle)] bg-[var(--surface)]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-5 py-4 xl:px-8">
          <Link href="/${v.name}/search" className="flex items-center gap-2 text-sm font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
      ...
      <div className="mx-auto max-w-[1380px] px-5 pt-8 xl:px-8">
        ...
        <div className="grid gap-4 overflow-hidden rounded-[2rem] sm:grid-cols-4 md:h-[60vh] xl:h-[70vh]">
          <div className="relative col-span-1 sm:col-span-2 md:col-span-3 overflow-hidden group">
            <img src={heroImage} alt={listing.${v.titleKey} || 'Item'} className="absolute inset-0 h-full w-full object-cover transition-transform duration-1000 group-hover:scale-105" />
          </div>
        ...
        <div className="mt-12 grid gap-12 lg:grid-cols-[1fr_400px]">
          ...
          <div className="space-y-12">
            <div>
              ...
              <h1 className="mt-6 text-3xl font-bold tracking-tight text-[var(--text-primary)] md:text-5xl">{listing.${v.titleKey}}</h1>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              ...
              <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-alt)] p-4 text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">${spec.label}</p>
                <p className="mt-2 text-base font-semibold text-[var(--text-primary)]">{listing.${spec.valueKey} || '-'}</p>
              </div>
              `).join('')}
            </div>
          ...
          <div>
            <div className="sticky top-28 rounded-[2rem] border border-[var(--border-default)] bg-[var(--surface)] p-8 shadow-[var(--shadow-md)]">
              ...
              <h2 className="mt-2 text-4xl font-bold tracking-tight text-[var(--text-primary)]">{listing.${v.priceKey} || 'Price on request'}</h2>
              ...
              <div className="mt-8 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button className="flex items-center justify-center rounded-2xl ${v.color} px-6 py-4 text-sm font-bold text-white transition hover:brightness-110 shadow-lg">
                    Call Agent
                  </button>
