...
import { getClassifiedsListingBySlug } from '../../../../lib/search/classifieds';
...
export default async function DetailPage({ params }: Props) {
  ...
  const listing = getClassifiedsListingBySlug(resolvedParams.slug);
    ...
    <div className="bg-[var(--background)] pb-24 animate-in fade-in duration-700">
      ...
      <div className="sticky top-0 z-50 border-b border-[var(--border-subtle)] bg-[var(--surface)]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-5 py-4 xl:px-8">
          <Link href="/classifieds/search" className="flex items-center gap-2 text-sm font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              ...
              <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-alt)] p-4 text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Condition</p>
                <p className="mt-2 text-base font-semibold text-[var(--text-primary)]">{listing.condition || '-'}</p>
              </div>
              ...
              <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-alt)] p-4 text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Delivery</p>
                <p className="mt-2 text-base font-semibold text-[var(--text-primary)]">{listing.delivery || '-'}</p>
              </div>
            ...
            <div className="sticky top-28 rounded-[2rem] border border-[var(--border-default)] bg-[var(--surface)] p-8 shadow-[var(--shadow-md)]">
              ...
              <div className="mt-8 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button className="flex items-center justify-center rounded-2xl bg-purple-500 px-6 py-4 text-sm font-bold text-white transition hover:brightness-110 shadow-lg">
                    Call Agent
                  </button>
