...
import { getMotorListingBySlug } from '../../../../lib/search/motors';
...
export default async function MotorDetailPage({ params }: Props) {
  ...
  const listing = getMotorListingBySlug(resolvedParams.listingSlug);

  if (!listing) {
    notFound();
  }

  // Use the provided imageUrl or fallback to a stunning car hero image
  const heroImage = listing.imageUrl || 'https://images.unsplash.com/photo-1503376712344-652bb8fc59eb?auto=format&fit=crop&w=1600&q=80';
  const subImage1 = 'https://images.unsplash.com/photo-1549399542-7e3f8b79c341?auto=format&fit=crop&w=800&q=80';
  const subImage2 = 'https://images.unsplash.com/photo-1542282088-fe8426682b8f?auto=format&fit=crop&w=800&q=80';
    ...
    <div className="bg-[var(--background)] pb-24">
      ...
      <div className="sticky top-0 z-50 border-b border-[var(--border-subtle)] bg-[var(--surface)] backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1380px] items-center justify-between px-5 py-4 xl:px-8">
          <Link href="/motors/search" className="flex items-center gap-2 text-sm font-semibold text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
        ...
        <div className="grid gap-4 overflow-hidden rounded-[2rem] sm:grid-cols-4 md:h-[60vh] xl:h-[70vh]">
          ...
          <div className="hidden grid-rows-2 gap-4 sm:grid md:col-span-1">
            <div className="relative">
              <img src={subImage1} alt="Car interior" className="absolute inset-0 h-full w-full object-cover" />
            </div>
            <div className="relative">
              <img src={subImage2} alt="Car rear view" className="absolute inset-0 h-full w-full object-cover" />
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-alt)] p-4 text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Year</p>
                <p className="mt-2 text-base font-semibold text-[var(--text-primary)]">{listing.year}</p>
              </div>
              <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-alt)] p-4 text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Mileage</p>
                <p className="mt-2 text-base font-semibold text-[var(--text-primary)]">{listing.mileage}</p>
              </div>
              <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-alt)] p-4 text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Body</p>
                <p className="mt-2 text-base font-semibold text-[var(--text-primary)]">{listing.vehicleType}</p>
              </div>
              <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-alt)] p-4 text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">History</p>
                <p className="mt-2 text-base font-semibold text-[var(--text-primary)]">Clean Title</p>
              </div>
            </div>
            ...
            {/* Description */}
            <div>
              <h2 className="text-xl font-bold tracking-tight text-[var(--text-primary)]">Vehicle Details</h2>
              <p className="mt-6 leading-8 text-[var(--text-secondary)]">
                {listing.summary}
                <br /><br />
                This meticulously maintained {listing.year} {listing.vehicleType} represents the pinnacle of engineering and luxury. Finished in an elegant exterior color with a premium interior, it offers a seamless blend of performance and comfort.
                <br /><br />
                Key highlights:
                <br />• GCC Specification
                <br />• Full service history at the authorized agency
                <br />• Comprehensive insurance available
                <br />• Fully sanitized and detailed prior to handover
              </p>
            </div>
          ...
          <div>
            <div className="sticky top-28 rounded-[2rem] border border-[var(--border-default)] bg-[var(--surface)] p-8 shadow-[var(--shadow-md)]">
              ...
              <p className="mt-2 text-sm font-medium text-[var(--text-secondary)]">
                Finance from AED {(parseInt(listing.priceLabel.replace(/[^0-9]/g, ''), 10) / 60).toFixed(0)} / month
              </p>
              ...
              <div className="mt-8 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button className="flex items-center justify-center rounded-2xl bg-[var(--motors-accent)] px-6 py-4 text-sm font-bold text-[var(--text-inverse)] transition hover:bg-orange-700">
                    Call Seller
                  </button>
                ...
                <div className="mt-4 flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-[var(--text-primary)] flex items-center gap-1">
                      {listing.sellerName}
                      {listing.sellerType.includes('Verified') && <CheckBadgeIcon className="h-5 w-5 text-[#2563eb]" />}
                    </p>
